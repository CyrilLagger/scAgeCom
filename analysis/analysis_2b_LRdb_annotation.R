...
## 0. Library installation (to do only once and then this part can be removed) ####

#very convenient package that extend the "data.frame" type
install.packages("data.table")

#"biomarRT" give a way to query Ensembl information directly from R (belongs to BioConductor, so require BiocManager)
...
#retrive the information we want for our genes of interest
...
saveRDS(LR_genes_info, "../data_scAgeCom/analysis/analysis_2_LR_genes_info.rds")
...
## 6. Combine the GO terms (one row per gene)
...
saveRDS(LR_genes_go, "../data_scAgeCom/analysis/analysis_2_LR_GO_terms.rds")

## 7. Combine the family terms (one row per gene)
...
saveRDS(LR_genes_family, "../data_scAgeCom/analysis/analysis_2_LR_family_terms.rds")

## 8. Merge the two previous tables in a single table
...
saveRDS(LR_genes_go_family, "../data_scAgeCom/analysis/analysis_2_LR_GO-family_terms.rds")


## 9. Create a table of the GO/family term frequency ####
...
#each table can be filtered as followed
#LR_go_frequency_filt <- LR_go_frequency[N >= 7]


##### Below: code for testing ####

## 10. On how to extract group of genes with specific terms ####
