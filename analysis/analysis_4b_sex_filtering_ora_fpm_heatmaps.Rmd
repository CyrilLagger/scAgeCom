...
DIFFCOM_RESULT_PATH <- list(
  ...
  tms_droplet = "../../data_scAgeCom/scDiffCom_sex_results/diffcom_sex_tms_droplet_size_factor_log_10000iter_mixed"
)
...
cols = get_default_sex_colnames()

datasets <- lapply(
  datasets,
  function(data) {
    cutoff <- quantile(c(data$LR_SCORE_male, data$LR_SCORE_female), 0.75)
    data <- analyze_CCI(
      data,
      cutoff_score = cutoff,
      cols = cols
