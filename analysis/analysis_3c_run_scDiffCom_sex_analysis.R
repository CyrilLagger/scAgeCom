...
##
## Apply scDiffCom to TMS datasets to study
...
if(is_log) {
  if(calico_subtype) {
    output_dir <- paste0(dir_data_output, "/diffcom_sex_", dataset, "_subtype_", normalization, "_log_", n_iter, "iter_", LR_db)
  } else {
    output_dir <- paste0(dir_data_output, "/diffcom_sex_", dataset, "_", normalization, "_log_", n_iter, "iter_", LR_db)
  }
} else {
  if(calico_subtype) {
    output_dir <- paste0(dir_data_output, "/diffcom_sex_", dataset, "_subtype_", normalization, "_nlog_", n_iter, "iter_", LR_db)
  } else {
    output_dir <- paste0(dir_data_output, "/diffcom_sex_", dataset, "_", normalization, "_nlog_", n_iter, "iter_", LR_db)
  }
}
if(run_test) {
...
foreach(i = 1:n_tissue, .packages = c("Seurat", "scDiffCom")) %dopar% {
  ...
  if(normalization == "size_factor") {
    ...
    saveRDS(dt_res, file = paste0(output_dir, "/diffcom_sex_", tiss, ".rds"))
